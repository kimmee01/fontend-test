...
const FormTicket: FC<ITicketData> = (props) => {
    const [dataInitial, setDataInitial] = useState(props.data)
    ...
    const onSubmit = async (data: ITicketDataPost) => {
        const response = {
            id: dataInitial?.id || null,
            name: data.name,
            email: data.email,
    ...
    return (
        <Styled>
            <form
                onSubmit={handleSubmit((data) => { onSubmit(data as ITicketDataPost) })} method="post"
            >
                <div className="row">
